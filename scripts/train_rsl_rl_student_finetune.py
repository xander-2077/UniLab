...
def _resolve_aux_checkpoint(cfg: DictConfig, section: str) -> tuple[Path | None, Path | None]:
    ...
    return resolve_task_checkpoint_path(
        ...
        checkpoint=_optional_checkpoint_value(OmegaConf.select(section_cfg, "checkpoint", default=-1)),
        log_root=OmegaConf.select(section_cfg, "log_root", default=None),
    )


def _resolve_run_config_path(load_run: str | None) -> Path | None:
    if not load_run or load_run in {"", "-1"}:
        return None
    candidate = Path(load_run)
    if not candidate.is_absolute():
        candidate = ROOT_DIR / candidate
    if candidate.is_dir():
        candidate = candidate / "run_config.json"
    return candidate if candidate.exists() else None


def _apply_reward_source(cfg: DictConfig) -> Path | None:
    load_run = OmegaConf.select(cfg, "reward_source.load_run", default=None)
    run_config_path = _resolve_run_config_path(str(load_run) if load_run is not None else None)
    if run_config_path is None:
        if bool(OmegaConf.select(cfg, "reward_source.strict", default=False)):
            raise FileNotFoundError(f"Could not resolve reward_source.load_run={load_run!r}")
        return None

    with run_config_path.open("r", encoding="utf-8") as f:
        run_config = json.load(f)
    reward_cfg = run_config.get("config", {}).get("reward")
...
def _load_student_actor_from_distill(
    ...
    *,
    ...
    if "student_state_dict" not in loaded:
        raise KeyError(
            f"Distill checkpoint must contain student_state_dict: {distill_checkpoint}"
        )
...
@hydra.main(version_base="1.3", config_path="../conf/rsl_rl_student_finetune", config_name="config")
def main(cfg: DictConfig) -> None:
    ensure_registries()
    reward_source_path = _apply_reward_source(cfg)
    ...
    log_dir = str(Path(log_root) / cfg.training.task_name / f"{timestamp}_{cfg.training.sim_backend}")
    ...
    try:
        ...
        if resume_path is not None:
            print(f"Resuming PPO fine-tune from {resume_path}")
        ...
        train_summary = {
            ...
            "last_checkpoint": str(Path(log_dir) / f"model_{int(runner.current_learning_iteration)}.pt"),
            "distill_checkpoint": str(distill_checkpoint) if distill_checkpoint else None,
            "teacher_checkpoint": str(teacher_checkpoint),
            "reward_source": str(reward_source_path) if reward_source_path else None,
